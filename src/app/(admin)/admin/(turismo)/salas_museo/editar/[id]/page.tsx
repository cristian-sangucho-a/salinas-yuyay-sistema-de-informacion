...
export default function EditarSalaPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const contentFromServer = String(sala.contenido ?? "");
        setEslogan(String(sala.eslogan ?? ""));
        setContenido(contentFromServer);
        setPublico(
          sala.publico === undefined ? undefined : Boolean(sala.publico)
        );
        ...
        const portadaUrl = generarUrlImagen(
          sala.collectionId,
          sala.id,
          sala.portada
        );
        ...
        if (Array.isArray(rawGaleria)) {
          const urls = (rawGaleria as unknown[])
            .map((filename) => {
              return generarUrlImagen(
                sala.collectionId,
                sala.id,
                filename as string
              );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!titulo || !eslogan || !editorContent) {
        ...
        <article className="max-w-4xl mx-auto my-8 relative">
          <div className="fixed bottom-8 right-8 z-50 flex items-center gap-4">
            {status && (
              <Alert
                type={
                  status.includes("Error") || status.includes("no encontrada")
                    ? "error"
