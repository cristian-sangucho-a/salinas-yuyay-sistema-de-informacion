  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setFechaFin(
          ev.fecha_de_finalizacion
            ? new Date(String(ev.fecha_de_finalizacion))
            : null
        );
          ...
          const urls = (rawGaleria as unknown[])
            .map((filename) => {
              ...
              return generarUrlImagen(
                tempRecord.collectionId,
                tempRecord.id,
                tempRecord.galeria as string
              );
            })
        ...
        <article className="max-w-4xl mx-auto my-8 relative">
          <div className="fixed bottom-8 right-8 z-50 flex items-center gap-4">
            {status && (
              <Alert
                type={
                  status.includes("Error") || status.includes("no encontrado")
                    ? "error"
