...
import { getSalaMuseoByIdServer, generarUrlImagenSala } from "@/lib/data/turismo/salas-museo-server";
...
export default async function SalaPage({ params }: Props) {
  ...
  let sala: SalaMuseo | null = await getSalaMuseoByIdServer(id)
      ...
      <article className="max-w-4xl mx-auto my-8">
        {sala.portada && (
          <div className="w-full h-72 md:h-96 overflow-hidden rounded-b-md">
            <img src={generarUrlImagenSala(sala.collectionId, sala.id, sala.portada)} alt={sala.titulo} className="w-full h-full object-cover" />
          </div>
        )}

        <div className="p-8">
          <h1 className="text-3xl md:text-4xl font-bold mb-3">{sala.titulo}</h1>
          {sala.resumen && <p className="text-base-content/70 mb-6">{sala.resumen}</p>}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                {sala.galeria.map((url, index) => (
                  <div key={`gallery-${index}`} className="h-44 overflow-hidden rounded shadow-sm">
                    <img src={generarUrlImagenSala(sala.collectionId, sala.id, url)} alt={`Imagen de galería ${index + 1}`} className="w-full h-full object-cover" />
                  </div>
                ))}
              </div>
            </section>
