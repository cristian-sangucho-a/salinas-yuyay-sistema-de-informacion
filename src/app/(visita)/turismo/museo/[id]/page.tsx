      ...
      <article className="max-w-4xl mx-auto my-8">
        {sala.portada && (
          <div className="w-full h-72 md:h-96 overflow-hidden rounded-b-md relative">
            <Image src={generarUrlImagenSala(sala.collectionId, sala.id, sala.portada)} alt={sala.titulo} fill className="object-cover" />
          </div>
        )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                {sala.galeria.map((url, index) => (
                  <div key={`gallery-${index}`} className="h-44 overflow-hidden rounded shadow-sm relative">
                    <Image src={generarUrlImagenSala(sala.collectionId, sala.id, url)} alt={`Imagen de galería ${index + 1}`} fill className="object-cover" />
                  </div>
                ))}
              </div>
            </section>
